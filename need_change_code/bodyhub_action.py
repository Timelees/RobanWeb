...
class Action(object):
    ...
    def __init__(self, name="NodeName", ctl_id = 2, init_node=True):
        
        if type(name) == bodycli.BodyhubClient:# 允许外部传入bodyclient实例
            print("using extern bodyhub client")
            self.bodyhub = name
        else:
            if init_node == True and type(name)==str:
    ...
    def __ros_shutdown_hook(self):
        if self.bodyhub.reset() == True:
            ...
            rospy.logwarn('reset failed, exit')

    def bodyhub_reset(self):
        if self.bodyhub.reset() == False:
            rospy.logerr('bodyhub to reset failed!')
